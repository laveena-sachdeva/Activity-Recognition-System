...
def apply_classifiers(rootdir, myoData):
    for dirs, subdirs, files in os.walk(rootdir+groundTruth):
            i=0
            train_eat_df = pd.DataFrame()
            train_noneat_df = pd.DataFrame()
            for s in subdirs:
                    working_dir = rootdir + myoData + "/" + s + "/fork"
                    for f in os.listdir(working_dir):
                        ...
                        if(fnmatch.fnmatch(f,'reducedfeaturematrix_eat_*.csv')):
                            ...
                            train_eat_df = train_eat_df.append(eating_df)
                      ...
                      #      print(eating_df.shape)        
                        ...
                        if(fnmatch.fnmatch(f,'reducedfeaturematrix_noneat_*.csv')):
                            ...
                            train_noneat_df = train_noneat_df.append(noneating_df)
                            #noneating_df = noneating_df.values
                            #noneat_count = noneating_df.shape[0]
                    ...
                    if(i==18):    
                        ...
                        train_eat_array = train_eat_df.values
                        train_noneat_array = train_noneat_df.values
                        
                        noTr_eat = int(len(train_eat_array))
                        #noTe_eat = len(train_eat_array) - noTr_eat
                        noTr_noneat = int(len(train_noneat_array))
                        #noTe_noneat = len(train_noneat_array) - noTr_noneat
                        
                        train_eat = train_eat_array
                        train_noneat = train_noneat_array
                        #test_eat = train_eat_array[noTr_eat: , :]
                        #test_noneat = train_noneat_array[noTe_noneat: , :]
...
###DST CLASSIFICATION STARTS HERE##########                
                    ...
                    if(i>18):
                        ...
                        test_eat = eating_df.values
                        test_noneat = noneating_df.values
    ...
    #                    print(re, end = ' ')
    #                    print(f)
                        final_metrics_dst = working_dir + "/phase3_dst_precision_recall_f1.csv"
    ...
    #                    print(re, end = ' ')
    #                    print(f)
                        final_metrics_svm = working_dir + "/phase3_svm_precision_recall_f1.csv"
    ...
    #                    print(re, end = ' ')
    #                    print(f)
                        final_metrics_nn = working_dir + "/phase3_nn_precision_recall_f1.csv"
